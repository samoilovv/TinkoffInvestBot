...
bool SinkFileLog::checkLogFileOpen()
{
  ...
  if(ENABLE_CONSOLE_LOG_FILE_STATE) {
    ...
    qDebug() << "Current log-file:" << _logFileName << "role" << _role << "version" << VERSION_HEVAA;
  ...
  if(!_startMessage) {
    _startMessage = true;
    slotLog(SimpleQtLogger::timeStamp(), SimpleQtLogger::threadId(), QString("Start file-log '%1' version-%2").arg(_role).arg(VERSION_HEVAA), LogLevel_INTERNAL, "", "", 0);
  }
